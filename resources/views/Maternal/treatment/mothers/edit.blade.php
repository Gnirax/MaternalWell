...
@section('content')
    ...
    <div id="treatment" class="container card shadow-lg p-3 mb-5 bg-body-tertiary rounded">
        <form method="POST" action="{{ route('treatment.update') }}">
            @csrf
            <div class="tab-content">
                <div id="observations" class="tab-pane fade">
                    <div class="form-group">
                        <input type="hidden" name="consultations_id" value="{{ $treatments->consultations_id }}">
                        <input type="hidden" name="type" value="mothers">
                        <input type="hidden" name="mothers_id" value="{{ $treatments->mothers_id }}">
                        <label class="form-label">OBSERVATIONS:</label>
                        <textarea name="observations" class="form-control" value="{{ $treatments->observations }}"></textarea>
                    </div>
                </div>
                <div id="hypothesis" class="tab-pane fade">
                    <div class="form-group">
                        <label class="form-label">HYPOTHESIS:</label>
                        <textarea class="form-control" name="hypothesis" value="{{ $treatments->hypothesis }}"></textarea>
                    </div>
                </div>
                <div id="diagnostic_tests" class="tab-pane fade">
                    <div class="form-group">
                        <label class="form-label">DIAGNOSTIC TESTS:</label>
                        <textarea class="form-control" name="diagnostic_tests" value="{{ $treatments->diagnostic_tests }}"></textarea>
                    </div>
                </div>
                <div id="diagnostic_results" class="tab-pane fade">
                    <div class="form-group">
                        <label class="form-label">DIAGNOSTIC RESULTS:</label>
                        <textarea class="form-control" name="diagnostics_results" value="{{ $treatments->diagnostics_results }}"></textarea>
                    </div>
                </div>
                <div id="diagnosis" class="tab-pane fade">
                    <div class="form-group">
                        <label class="form-label">DIAGNOSIS:</label>
                        <textarea class="form-control" name="diagnosis" value="{{ $treatments->diagnosis }}"></textarea>
                    </div>
                </div>
                <div id="medications" class="tab-pane fade">
                    <div class="form-group">
                        <label class="form-label">MEDICATIONS:</label>
                        <textarea class="form-control" name="medications" value="{{ $treatments->medications }}"></textarea>
                    </div>
                </div>
                <div id="treatment_plan" class="tab-pane fade">
                    <div class="form-group">
                        <label class="form-label">TREATMENT PLAN</label>
                        <textarea class="form-control" name="treatment_plan" value="{{ $treatments->treatment_plan }}"></textarea>
                    </div>
                </div>
            </div>
